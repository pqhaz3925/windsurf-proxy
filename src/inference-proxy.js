// inference-proxy.js — HTTP/2 inference proxy for Windsurf (port 3001)
//
...
const PORT = parseInt(process.env.INFERENCE_PORT || '3001', 10);
...
function forwardToCodeium(body, stream, headers, path, id) {
  ...
  const fwdReq = https.request({
    ...
    fwdRes.on('data', (chunk) => { if (!stream.destroyed) stream.write(chunk); });
    fwdRes.on('end', () => {
